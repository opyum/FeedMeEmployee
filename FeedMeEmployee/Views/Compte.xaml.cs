...
using TodoREST.Services; // Assurez-vous d'ajouter cette directive using si elle n'est pas déjà présente

namespace FeedMeEmployee.Views
{
    public partial class ComptePage : ContentPage
    {
        public ComptePage(IRestService restService)
        {
        ...
        NFCNdefTypeFormat _type = NFCNdefTypeFormat.Uri;
        ...
        {
            ShowAlert("Current_OnTagConnected");
            CrossNFC.Current.PublishMessage(new TagInfo
            {
                Records = new[] { new NFCNdefRecord
                {
                    ...
                    MimeType = MIME_TYPE,
                    Payload = NFCUtils.EncodeToByteArray("HAAAAAAAAAAAAAAAAAAAA"),
                    LanguageCode = "en"
                } }
            });
        ...
        {
            await ShowAlert("Current_OnMessageReceived" + isEnabled.ToString());
        ...
        {
            await ShowAlert("Current_OnMessageReceived");
            if (tagInfo == null)
            {
        ...
        {
            await ShowAlert("Current_OnMessagePublished");
        ...
        async void Current_OnTagDiscovered(ITagInfo tagInfo, bool format)
        {
            try
            {
                ...
                while(IsProcessing)
                {
                    var transactionList= await service.GetAllTransactions();
                    IsProcessing = transactionList.Any(x => x.DateHeure <= DateTime.UtcNow.AddMinutes(-1));
            ...
            {
                await ShowAlert($"Erreur lors de l'émission du message NFC : {ex.Message}");
            }
        }
        //public async void Current_OnTagDiscovered(ITagInfo tagInfo, bool format)
        //{
        //    await ShowAlert("Current_OnTagDiscovered");
        ...
        //        TypeFormat = NFCNdefTypeFormat.WellKnown,
        //        MimeType = MIME_TYPE,
        //        Payload = NFCUtils.EncodeToByteArray("Message à envoyer"),
                
        //    };

        //    var message = new TagInfo { Records = new[] { record } };

        //    try
        //    {
        //        CrossNFC.Current.PublishMessage(message);
        //    }
        //    catch (Exception ex)
        //    {
        //        await ShowAlert($"Erreur lors de l'émission du message NFC : {ex.Message}");
        ...
        async void Button_Clicked_MakePayment(object sender, System.EventArgs e)
        {
            // Afficher le pop-up avec le message
            //await DisplayAlert("Paiement NFC", "Approcher votre téléphone du terminal de paiement", "OK");
            IsProcessing = true;

            // Créer un nouveau NFCNdefRecord avec l'ID "1"
            var record = new NFCNdefRecord
            {
                TypeFormat = NFCNdefTypeFormat.WellKnown,
                MimeType = MIME_TYPE,
                Payload = NFCUtils.EncodeToByteArray("1")
            };

            // Créer un nouveau ITagInfo avec le NFCNdefRecord
            var tagInfo = new TagInfo
            {
                Records = new[] { record }
            };
            // Démarrer l'écoute des tags NFC avant d'émettre le message
            SubscribeEvents();

            CrossNFC.Current.StartPublishing(true);
            //CrossNFC.Current.StartPublishing(true);
            // Émettre l'événement NFC avec l'ID "1"
